...
class config:
    ...
    PATH_SCHEMA = "../model/medical_re/predicate.json"
    PATH_TRAIN = '../model/medical_re/train_example.json'  # train_example.json
    PATH_BERT = "../model/medical_re/"
    PATH_MODEL = "../model/medical_re/model_re.pkl"
    PATH_SAVE = '../model/medical_re/save'
    #tokenizer = BertTokenizer.from_pretrained("../model/medical_re/" + 'vocab.txt')
    tokenizer =  None#BertTokenizer.from_pretrained("../model/medical_re/" + 'vocab.txt')
    id2predicate = {}
    predicate2id = {}
...
def set_args():
    ...
    parser.add_argument( '--PATH_TRAIN', default='../model/medical_re/train_example.json', type=str, required=False, help='')
    parser.add_argument( '--PATH_SCHEMA', default="../model/medical_re/predicate.json", type=str, required=False, help='')
    parser.add_argument( '--PATH_BERT',   default="../model/medical_re/", type=str, required=False,  help='' )
    parser.add_argument( '--PATH_MODEL',  default='../model/medical_re/model_re.pkl', type=str, required=False, help='')
    parser.add_argument( '--PATH_SAVE',   default='../model/medical_re/save',  type=str, required=False, help='')
...
class Model4po(nn.Module):
    ...
    def forward(self, hidden_states, batch_subject_ids, input_mask):
        ...
        print('in batch_subject_ids b',batch_subject_ids)
        for b in range(hidden_states.shape[0]):
            print('in forward b',b)
...
def extract_spoes(text,tokenizer,max_seq_len,num_p,id2predicate, model4s, model4po):
    ...
    with torch.no_grad():
        ...
        end   = np.where(subject_labels_pred[0, :, 1] > 0.4)[0]  #返回满足条件的坐标
...
def run_train( PATH_SCHEMA, PATH_TRAIN,PATH_MODEL,PATH_SAVE,learning_rate,config ):
    ...
    gc.collect()
    # save
    if PATH_SAVE!=None or PATH_SAVE!='':
...
def load_model( PATH_SCHEMA,  PATH_MODEL ):
    load_schema(   PATH_SCHEMA  )
...
def get_triples(content,tokenizer,max_seq_len,num_p,id2predicate,model4s,model4po):
    if len(content) == 0:
        return []
    ...
    for text in text_list:
        if len(text) > 128:
            text = text[:128]
        triples = extract_spoes(text,tokenizer,max_seq_len,num_p,id2predicate, model4s, model4po)
        #extract_spoes(text,config,  model4s, model4po )
        res.append({
            'text': text,
            'triples': triples
        })
...
def set_config( args ):
    global config
    config.PATH_SCHEMA  = args.PATH_SCHEMA
    config.PATH_TRAIN   = args.PATH_TRAIN
    config.PATH_BERT    = args.PATH_BERT
    config.PATH_MODEL   = args.PATH_MODEL
    config.PATH_SAVE    = args.PATH_SAVE
    config.tokenizer = BertTokenizer.from_pretrained( args.PATH_BERT+ 'vocab.txt' ) #  "../model/medical_re/"
    return
set_config( args )
...
def train_re():
    global config
    run_train(  PATH_SCHEMA, PATH_TRAIN,PATH_MODEL,PATH_SAVE,learning_rate,config )
    # load_schema(args.PATH_SCHEMA)
    # model4s, model4po = load_model(args.PATH_SCHEMA, args.PATH_MODEL)
    # train(  train_data_loader, model4s, model4po, optimizer  )
    return
...
if __name__ == "__main__":
    ...
    run_train( config.PATH_SCHEMA, config.PATH_TRAIN, config.PATH_MODEL,config.PATH_SAVE,config.learning_rate ,config )
    #extract_data(   )
