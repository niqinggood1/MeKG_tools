...
class config:
    ...
    tokenizer = BertTokenizer.from_pretrained(PATH_BERT + 'vocab.txt') #"../model/medical_re/"
    #BertTokenizer.from_pretrained("../model/medical_re/" + 'vocab.txt')
    id2predicate = {}
    predicate2id = {}
...
def evaluate(data, is_print, tokenizer,max_seq_len,num_p,id2predicate,model4s, model4po):
    X, Y, Z = 1e-10, 1e-10, 1e-10
    for d in data:
        R = set([SPO(spo) for spo in extract_spoes(d['text'], tokenizer,max_seq_len,num_p,id2predicate,model4s, model4po)])  # 模型提取出的三元组数目
...
def run_train( PATH_SCHEMA, PATH_TRAIN,PATH_MODEL,PATH_SAVE,learning_rate,tokenizer,max_seq_len,num_p,id2predicate ):
    ...
    model4s.eval()
    model4po.eval()
    f1, precision, recall = evaluate(valid_data, True, tokenizer,max_seq_len,num_p,id2predicate,model4s, model4po)
...
if __name__ == "__main__":
    ...
    run_train( config.PATH_SCHEMA,   # 医学关系的json
               ...
               config.PATH_SAVE,     # 模型保存位置
               config.learning_rate ,# 学习率
               config.tokenizer,      #模型的
               config.max_seq_len,    #
               config.num_p,          # 医学关系数
               config.id2predicate    # 医学关系编码影射
               )
